"""Podcast suggestion, RSS feed, and episode management mixin for AppBackend."""
...
class PodcastMixin:
    ...
    def _load_podcasts(self) -> None:
        ...
        for idx, (feed, ep) in enumerate(all_eps):
            ...
            if ep.downloaded_path and Path(ep.downloaded_path).exists():
                wp = _compute_watched_percent(ep.downloaded_path)
            new_items.append({
                "title": ep.title,
    ...
    def openPodcastDetail(self, index: int) -> None:
        ...
        for i, ep in enumerate(feed.episodes):
            ...
            ep_items.append({
                ...
                "child_count": 0,
                "preview_images": [],
                "download_percent": 100.0 if (ep.downloaded_path and Path(ep.downloaded_path).exists()) else 0.0,
                "download_status": "done" if (ep.downloaded_path and Path(ep.downloaded_path).exists()) else "",
                "is_downloading": False,
                "watched_percent": wp,
    ...
    def playDownloadedPodcastEpisode(self, index: int) -> None:
        ...
        if path and Path(path).exists():
            ...
            self._play_item(item)
            return
        # File was cleaned up — re-download as watch-now
        suggestion_id = str(item.get("suggestion_id") or item.get("track_id") or "")
        source_url = str(item.get("url") or item.get("source_url") or "").strip()
        if not source_url:
            return
    ...
    def downloadPodcastEpisode(self, feed_url: str, guid: str) -> None:
        logger.info("[DEBUG downloadPodcastEpisode] feed_url=%s guid=%s", feed_url, guid)
        feeds = load_subscriptions()
        target_ep = None
        for feed in feeds:
        ...
        if target_ep is None:
            logger.warning("[DEBUG downloadPodcastEpisode] episode not found for feed=%s guid=%s", feed_url, guid)
            return
        if not target_ep.audio_url:
            logger.warning("[DEBUG downloadPodcastEpisode] episode has no audio_url: %s", target_ep.title)
            return

        if target_ep.downloaded_path and Path(target_ep.downloaded_path).exists():
            logger.info("[DEBUG downloadPodcastEpisode] already downloaded: %s", target_ep.downloaded_path)
            return

        logger.info("[DEBUG downloadPodcastEpisode] found episode: title=%s audio_url=%s", target_ep.title, target_ep.audio_url[:120])
        ...
        self.downloadJobStarted.emit(guid)
        asyncio.create_task(self._download_podcast_episode(
            target_ep.audio_url, feed_url, guid
        ))

    async def _download_podcast_episode(self, url: str, feed_url: str, guid: str) -> None:
        try:
            _video_id, file_path = await self._execute_download(url, "podcast")
            if Path(file_path).exists():
                update_episode_download(feed_url, guid, file_path)
                self._podcast_episode_model.update_download_state(
                    guid, percent=100.0, status="done", is_downloading=False,
                )
                self._podcast_new_episodes_model.update_download_state(
                    guid, percent=100.0, status="done", is_downloading=False,
                )
                self.downloadFinished.emit(guid, file_path)
                self._load_podcasts()
            else:
                self.downloadError.emit(guid, "Tải xuống hoàn tất nhưng không tìm thấy file.")
        except Download403Failed as exc:
            self.downloadError.emit(guid, str(exc))
        except DownloadFailed as exc:
            self.downloadError.emit(guid, str(exc))
        except Exception as exc:
            self.downloadError.emit(guid, f"Tải podcast thất bại: {exc}")
